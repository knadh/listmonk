<template>
  <section class="analytics content relative">
        ...
        <div class="column is-6">
          ...
          >
            <b-taginput
              ...
              autocomplete
              :allow-new="false"
              :before-adding="isCampaignSelected"
    ...
    <section class="charts mt-5">
          ...
          <div class="column is-9">
            ...
            <h4 v-if="v.chart !== null">
              ...
              <span class="has-text-grey-light"
                >({{ $utils.niceNumber(counts[k]) }})</span
              >
            </h4>
            <chart
              :type="v.type"
      ...
      <div class="mt-5">
        <h4>Individual Campaign Views</h4>
        <button
          type="button"
          class="button is-primary mb-3"
          @click="exportToCSV(`table-individual-views`)"
        >
          Export to CSV
        </button>
        <table
          class="table is-striped is-fullwidth"
          :id="`table-individual-views`"
        >
          <thead>
            <tr>
              <th>Name</th>
          ...
          <tbody>
            <tr
              v-for="(item, index) of tables.individualViews.data"
              :key="index"
            >
              ...
              <td>{{ item.email }}</td>
              <td>
                {{ item.status == "enabled" ? "Subscribed" : "Un-Subscribed" }}
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="mt-5">
        <h4>Individual Link Clicks</h4>
        <button
          type="button"
          class="button is-primary mb-3 outline"
          @click="exportToCSV_NOTAB(tables.individualClickUsers.data)"
        >
          Export to CSV
        </button>
        <table
          class="table is-striped is-fullwidth"
          :id="`table-individual-clicks`"
        >
          <thead>
            <tr>
              <th>Link</th>
          ...
          <tbody>
            <tr
              v-for="(item, index) of tables.individualClicks.data"
              :key="index"
            >
              <!-- <td>{{ item.campaign_id }}</td> -->
              <td>{{ item.url }}</td>
...
export default Vue.extend({
  ...
  methods: {
    ...
    makeCharts(typ, campaigns, data) {
      ...
      const points = campIDs.map((id) => {
        ...
        const sum = data.reduce(
          (a, item) => (item.campaignId === cId ? a + item.count : a),
          0
        );
        return sum;
      });
    ...
    onLinkClick(e) {
      const bars = e.chart.getElementsAtEventForMode(
        e,
        "nearest",
        { intersect: true },
        true
  ...
  created() {
    ...
    const from = this.$route.query.from
      ? dayjs.unix(this.$route.query.from)
      : weekAgo;
  ...
  mounted() {
    ...
    if (ids.length > 0) {
      this.isSearchLoading = true;
      Promise.allSettled(ids.map((id) => this.$api.getCampaign(id))).then(
        (data) => {
          data.forEach((d) => {
