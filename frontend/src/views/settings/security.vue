...
export default Vue.extend({
  ...
  computed: {
    ...
    captchaEnabled: {
      get() {
        return this.data['security.captcha'].hcaptcha.enabled || this.data['security.captcha'].altcha.enabled;
      },
      set(value) {
        this.data['security.captcha'].hcaptcha.enabled = !!value;
    ...
    selectedProvider: {
      get() {
        if (this.data['security.captcha'].hcaptcha.enabled) {
          return 'hcaptcha';
        } if (this.data['security.captcha'].altcha.enabled) {
          return 'altcha';
        }
        return 'altcha';
      },
  ...
  methods: {
    ...
    onCaptchaToggle(enabled) {
      if (enabled) {
        // Initialize captcha structure if it doesn't exist
        if (!this.data['security.captcha']) {
          this.$set(this.data, 'security.captcha', {
            hcaptcha: { enabled: false, key: '', secret: '' },
            altcha: { enabled: false, complexity: 1000 },
          });
        }
        // Enable default provider (hcaptcha)
        this.selectedProvider = 'hcaptcha';
      }
    },
  },
