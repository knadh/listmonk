<template>
  <section class="bounces">
    ...
    <div class="card page-content">
      <b-table :data="bounces.results" :hoverable="true" :loading="loading.bounces" default-sort="createdAt"
          ...
          <div class="actions">
            <template v-if="bulk.checked.length > 0">
              ...
              <a class="a" href="#" @click.prevent="$utils.confirm(null, () => blocklistSubscribers())"
                ...
                <b-icon icon="account-off-outline" /> {{ $t('import.blocklist') }}
              </a>
              <span>
        ...
        <b-table-column v-slot="props" field="email" :label="$t('subscribers.email')" :td-attrs="$utils.tdID"
          sortable>
          <router-link :to="{ name: 'subscriber', params: { id: props.row.subscriberId } }"
            :class="{ 'blocklisted': props.row.subscriberStatus === 'blocklisted' }">
            {{ props.row.email }}
            <b-tag v-if="props.row.subscriberStatus !== 'enabled'" :type="props.row.subscriberStatus"
              data-cy="blocklisted">
