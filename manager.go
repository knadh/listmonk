...
type Manager struct {
    ...
    // Sending window state: limits sends to a configured time window and daily cap.
    sendingWindowStart time.Duration
    sendingWindowEnd   time.Duration
    dailyMu            sync.Mutex
    dailyDate          time.Time
    dailySent          int
...
type Config struct {
    ...
    // SendingWindow enables time-based sending window enforcement.
    SendingWindowEnabled bool
    // SendingWindowStart and End are times represented as durations since midnight.
    SendingWindowStart time.Duration
    SendingWindowEnd   time.Duration
    // Daily sending cap when sending window enabled. 0 means no cap.
    SendingWindowDailyCap int
}
...
func New(cfg Config, store Store, i *i18n.I18n, l *log.Logger) *Manager {
    ...
    // initialize sending window state
    m.sendingWindowStart = cfg.SendingWindowStart
    m.sendingWindowEnd = cfg.SendingWindowEnd
    m.dailyDate = time.Now().UTC()
    m.dailySent = 0
...
func (m *Manager) worker() {
    ...
    for {
        ...
        case msg, ok := <-m.campMsgQ:
            ...
            // If sending window is enabled, check whether current local time
            // is within the allowed window. If not, sleep until window start.
            if m.cfg.SendingWindowEnabled {
                now := time.Now()
                // Convert now to duration since midnight.
                midnight := time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, now.Location())
                since := now.Sub(midnight)

                // If daily rollover, reset daily counter.
                m.dailyMu.Lock()
                if now.UTC().Truncate(24 * time.Hour).After(m.dailyDate.Truncate(24 * time.Hour)) {
                    m.dailyDate = now.UTC()
                    m.dailySent = 0
                }
                // Enforce daily cap if set.
                if m.cfg.SendingWindowDailyCap > 0 && m.dailySent >= m.cfg.SendingWindowDailyCap {
                    // Sleep until next day.
                    sleepDur := midnight.Add(24 * time.Hour).Sub(now)
                    m.dailyMu.Unlock()
                    time.Sleep(sleepDur)
                    // After sleep, reset counters and continue.
                    m.dailyMu.Lock()
                    m.dailyDate = time.Now().UTC()
                    m.dailySent = 0
                    m.dailyMu.Unlock()
                ...
                // If current time is before window start or after window end, wait.
                // Handles windows that wrap around midnight (end < start).
                start := m.sendingWindowStart
                end := m.sendingWindowEnd
                inWindow := false
                if start <= end {
                ...
                if !inWindow {
                    // compute duration until next window start
                    var until time.Duration
                    if since < start {
                        until = start - since
                    } else {
                        // after end of today, wait until tomorrow's start
                        until = (24 * time.Hour) - since + start
                    }
                    time.Sleep(until)
                    // after waking up, continue to process messages
                }
            }
            numMsg++
            ...
            if msg.pipe != nil {
                ...
                if err != nil {
                    ...
                    msg.pipe.sent.Add(1)
                    // Increment global daily counter if sending window enabled.
                    if m.cfg.SendingWindowEnabled {
                        m.dailyMu.Lock()
                        m.dailySent++
                        m.dailyMu.Unlock()
