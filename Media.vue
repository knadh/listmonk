          ...
          <div>
            <div class="field is-grouped">
              <div class="control is-expanded">
                <b-field :label="$t('media.folder')">
                  <b-input v-model="form.folder" name="folder" placeholder="folder/subfolder" />
                </b-field>
              </div>
              <div class="control">
                <b-button type="is-info" @click.prevent="onCreateFolder">{{ $t('media.createFolder') }}</b-button>
              </div>
            </div>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      for (let i = 0; i < this.toUpload; i += 1) {
        ...
        if (this.form.folder && this.form.folder !== '') {
          params.set('folder', this.form.folder);
        }
    ...
    async onCreateFolder() {
      if (!this.form.folder || this.form.folder.trim() === '') return;
      const data = { folder: this.form.folder };
      try {
        await this.$api.createMediaFolder(data);
        this.$buefy.toast.open({ message: this.$t('media.folderCreated'), type: 'is-success' });
      } catch (e) {
