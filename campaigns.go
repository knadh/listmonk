...
func (c *Core) GetCampaignAnalyticsCounts(campIDs []int, typ, fromDate, toDate string) ([]models.CampaignAnalyticsCount, error) {
	// Pick campaign view/click/bounce counts. For views/clicks, choose unique
	// or non-unique prepared statements based on the current settings.
	var stmt *sqlx.Stmt
	switch typ {
	case "views":
		// decide unique vs non-unique at runtime
	settings, err := c.GetCachedSettings()
		if err != nil {
			return nil, err
		}
		if settings.PrivacyIndividualTracking {
...
func (c *Core) GetCampaignAnalyticsLinks(campIDs []int, typ, fromDate, toDate string) ([]models.CampaignAnalyticsLink, error) {
	...
	// Choose distinct vs non-distinct link counts based on settings.
	settings, err := c.GetCachedSettings()
	if err != nil {
		return nil, err
	}
	if settings.PrivacyIndividualTracking {
		if err := c.q.GetCampaignLinkCountsDistinct.Select(&out, pq.Array(campIDs), fromDate, toDate); err != nil {
