...
func V5_1_0(db *sqlx.DB, fs stuffbin.FileSystem, ko *koanf.Koanf, lo *log.Logger) error {
	if _, err := db.Exec(`
	ALTER TABLE campaigns ADD sliding_window bool NOT NULL DEFAULT false;
	ALTER TABLE campaigns ADD sliding_window_rate int NOT NULL DEFAULT 1;
	ALTER TABLE campaigns ADD sliding_window_duration varchar(4) NOT NULL DEFAULT '1h';
