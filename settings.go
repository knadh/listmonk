...
func (a *App) UpdateSettings(c echo.Context) error {
	...
	// Refresh in-memory cached settings in core so runtime paths pick up changes
	if err := a.core.RefreshSettingsCache(); err != nil {
		// Log but don't block the response; reload will pick up changes.
		a.log.Printf("warning: failed to refresh settings cache: %v", err)
	}
