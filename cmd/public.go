...
// appendUTMParams adds or overrides UTM query params on destURL and preserves the fragment.
// Returns the new URL and true, or ("", false) on parse error so the caller can redirect to the original URL.
func appendUTMParams(destURL, campaignName, utmSource, utmMedium, utmContent string) (string, bool) {
	...
	q.Set("utm_campaign", campaignName)
	q.Set("utm_term", time.Now().Format("2006-01-02"))
	q.Set("utm_content", utmContent)
	u.RawQuery = q.Encode()
	// Fragment is preserved: url.Parse puts it in u.Fragment, and String() rebuilds correctly.
...
// When app.utm is enabled and utm_source is set, redirect URL gets UTM parameters appended.
func (a *App) LinkRedirect(c echo.Context) error {
	...
	if a.cfg.UTM.Enabled && a.cfg.UTM.Source != "" {
		...
		content := a.cfg.UTM.Content
		if u, ok := appendUTMParams(destURL, campaignName, a.cfg.UTM.Source, medium, content); ok {
			destURL = u
		}
	}
