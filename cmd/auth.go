...
func (a *App) TwofaPage(c echo.Context) error {
	...
	// Validate the 2FA temp token (don't delete it yet, as we may need it for POST).
	...
	// Process POST 2FA verification request.
	if c.Request().Method == http.MethodPost {
...
func (a *App) doLogin(c echo.Context) error {
	...
	if user.TwofaType == models.TwofaTypeTOTP {
		...
		next := utils.SanitizeURI(c.FormValue("next"))
		return c.Redirect(http.StatusFound, fmt.Sprintf("/admin/login/twofa?token=%s&next=%s", token, url.QueryEscape(next)))
	}
...
func (a *App) doTwofaVerify(c echo.Context, token string, userID int, next string) error {
	...
	if !strHasLen(totpCode, 6, 6) {
		return a.renderTwofaPage(c, token, next, a.i18n.T("users.invalidTOTPCode"))
	}
	...
	// Verify that TOTP is still enabled for the user.
	if user.TwofaType != models.TwofaTypeTOTP {
	...
	// Consume the token now that 2FA verification succeeded.
