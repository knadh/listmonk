...
var regTplFuncs = []regTplFunc{
	// Transform plain <a href="url"> to TrackLink. [^"{}]+ skips template vars ({{ }}).
	{
		regExp:  regexp.MustCompile(`<a\s+href="([^"{}]+)"`),
		replace: `<a href="{{ TrackLink "$1" . }}"`,
	},
	// Same for single-quoted href='url'.
	{
		regExp:  regexp.MustCompile(`<a\s+href='([^'{}]+)'`),
		replace: `<a href='{{ TrackLink "$1" . }}'`,
	},
