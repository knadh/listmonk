...
func (c *Core) QueryMedia(provider string, s media.Store, query string, offset, limit int, folder string) ([]media.Media, int, error) {
	out := []media.Media{}
	...
	// Normalize folder prefix param. If provided, ensure no leading slash.
	if folder != "" {
		folder = strings.Trim(folder, "/\\")
	}

	if err := c.q.QueryMedia.Select(&out, fmt.Sprintf("%%%s%%", query), provider, offset, limit, folder); err != nil {
